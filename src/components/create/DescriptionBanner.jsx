...
const texts = [
  {
    ...
    tip: "Use keywords that represent themes or emotions, such as 'Love', 'Dream', or 'Adventure'. Be as descriptive as possible in the story section for the best results",
  },
  {
    title: 'Melody Maker',
  ...
  },
  {
    ...
    notice:
      'Transform your song into a visual masterpiece. Based on your keywords, genre, and style, AI will generate a unique album cover that reflects the mood and theme of your song. Customize the colors, textures, and layout to make the design truly yours. Your album cover will represent your music in the most captivating way.',
...
const DescriptionBanner = ({ pageNumber }) => {
  ...
  return (
    <div className="create__description-banner  mb40">
      {title && <p className="desc-banner__title">{t(title)}</p>}
      {strong && <strong className="desc-banner__strong">"{t(strong)}"</strong>}
      {notice && <p className="desc-banner__notice">{t(notice)}</p>}
      {tip && (
        <div className="desc-banner__tip-box">
          <p className="desc-banner__tip-header">{t('Tip')} : </p>
          <p className="desc-banner__tips">{t(tip)}</p>
        </div>
      )}
    </div>
  );
