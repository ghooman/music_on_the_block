...
const generateAlbumCoverPrompt = (lyricData, lyricStory) => {
    ...
    return `
      ...
      - 이미지에는 위의 키워드들을 반영하여, 예를 들어 "${lyric_tag.join(', ')}"와 "${lyric_genre.join(
        ', '
    )}"의 느낌을 표현할 것.
...
const MelodyMaker = ({
    ...
    const generateFinalPrompt = async () => {
        try {
            ...
            } ${standardizedGenre} song at ${tempo} BPM with ${melody_instrument?.join(', ') || ''}.`.substring(0, 200);
    ...
    return (
        <div className="create__melody-maker">
            ...
            {showLyricsModal && <LyricsModal setShowLyricsModal={setShowLyricsModal} generatedLyric={generatedLyric} />}
        </div>
    );
};
