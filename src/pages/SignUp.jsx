  ...
  const handleArtistNameCheck = async () => {
    ...
    try {
      ...
      if (response.data === true) {
        ...
        setPassName(false);
        setNameErrorMessage('This name is already in use. Please choose a different one.');
      }
    } catch (error) {
...
const PreferredGenre = ({ onBack, onNext, selectedGenre, setSelectedGenre }) => {
  const genreImages = {
    'K-POP': kpopImg,
    POP: popImg,
    'R&B': randbImg,
...
function SignUp() {
  const { token, walletAddress, isRegistered, setIsRegistered } = useContext(AuthContext);
  ...
  useEffect(() => {
    if (!walletAddress) {
      ...
      console.log('이미 회원가입이 완료된 사용자입니다. 메인 페이지로 이동합니다.');
      navigate('/');
    }
  ...
  const handleSignUp = async () => {
    const walletAddrString =
      walletAddress && walletAddress.address ? String(walletAddress.address) : '';
    try {
